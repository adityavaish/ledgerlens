...
Pivot drops a chat task pane into Excel that lets finance and accounting users:
...
Pivot runs entirely on the user's machine. A small launcher (`pivot`) starts a local Node server on a random free port, generates an Excel add-in manifest pointing at that port, and sideloads it into Excel. The server hosts the taskpane bundle, proxies Copilot SDK chat, runs Azure Data Explorer queries directly via `azure-kusto-data`, and brokers any local stdio MCP servers the user configures.

```
Excel (taskpane @ http://localhost:<port>)
  └─► Local Node server (started by `pivot`)
...
```
bin/
└── pivot.js   self-updating launcher (download/extract/spawn/sideload)
src/
...
The easiest way is the [**Pivot install page**](https://adityavaish.github.io/pivot/) — click **Install for Windows**, run the downloaded `Install-Pivot.cmd`, then launch from the Start-menu shortcut it creates.
...
1. Check `https://api.github.com/repos/adityavaish/pivot/releases/latest` and upgrade in place if a newer version is published. Skip with `PIVOT_SKIP_UPDATE=1`.
2. Pick a free localhost port (override with `PIVOT_PORT=3002`).
...
Excel opens with a Pivot button on the Home tab. Click it to open the taskpane and start chatting. Stop the server with `Ctrl+C`.
...
If `bin/pivot.js` is run from a repo checkout and no released version is cached yet, it runs the server out of the checkout directly. Set `PIVOT_SKIP_UPDATE=1` when iterating locally so it doesn't pull a different version on every run.
...
Tag a commit `vX.Y.Z` and push the tag — `.github/workflows/release.yml` will build, run `npm pack`, and publish a `pivot-X.Y.Z.tgz` plus the `install.ps1` script to the GitHub Release. The launcher fetches whichever release is marked **latest** on every run.

### Legacy: Azure App Service deployment

Earlier versions of Pivot were hosted on Azure App Service for Containers. That deployment is still functional but is no longer the primary distribution channel — per-user auth to Azure Data Explorer from a shared cloud host requires an Entra app registration plus admin consent, which the local-runner architecture neatly avoids. The `Dockerfile`, `azure.yaml`, and `infra/` directories remain in the tree for anyone who needs that path.
